            ...
            if ($this->form_validation->run($this) == TRUE) {
                ...
                if (!empty($userdetails)) {
                    if ($userdetails[0]['status'] == 1) {
                        ...
                        $session_datas = array('pms_err' => '1', 'pms_err_message' => 'Email/Password is wrong. Please try again', 'validation' => '1');
                        $this->session->set_userdata($session_datas);
						redirect(frontend_url());
                    }
                } else {
                    $session_datas = array('pms_err' => '1', 'pms_err_message' => 'Email/Password is not available. Please try again', 'validation' => '1');
                    $this->session->set_userdata($session_datas);
					redirect(frontend_url());
                }
            } else {
                $session_datas = array('pms_err' => '1', 'pms_err_message' => validation_errors(), 'validation' => '1');
                $this->session->set_userdata($session_datas);
				redirect(frontend_url());
            }
        } else {

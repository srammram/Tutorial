...
$(function () {
    return $('#emp_country,#asign_departments,#asign_user_details,#asign_project,#asign_usertype,#emp_departments,#user_type,#emp_state,#emp_city,#emp_accessmenu').select2(
            /*{
...
function checkstatus_withfinished(status_id) {
    if (status_id == 5) {
        $('#finished_div').show().slideDown();
        $('#add_finished_duration_hours').attr('required', 'required');
        $('#add_finished_message').attr('required', 'required');
    } else {
